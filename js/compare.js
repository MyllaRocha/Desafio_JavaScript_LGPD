...
function SetCarToCompare(el, carClass) {
    ...
    if(carClass instanceof Car){       
        if(el.checked){
                if (carArr.length >= 2){
                    alert ("Você pode comparar 2 veículos por vez!");
                    el.checked = false;
                    return;
                }
            }
...
function UpdateCompareTable() {
    for (let i = 0; i < carArr.length; i++) {
        document.getElementById(`compare_image_${i}`).innerHTML = `<img src="${carArr[i].image}" style="width: 100px;">`;
        ...
        document.getElementById(`compare_preco_${i}`).innerText = `R$ ${carArr[i].preco}`;
    }
